...
```{r colored_tips, message=FALSE, warning=FALSE}
library(scales)   # for col_numeric
library(phytools)

# 1. Create a color palette
colors <- colorRampPalette(c("lightyellow", "orange", "red"))(100)

# 2. Scale trait values into 1–100
scaled_trait <- as.numeric(
  cut(trait,
      breaks = 100,
      labels = FALSE,
      include.lowest = TRUE)
...
# 3. Assign colors
tip_colors <- colors[scaled_trait]

# 4. Plot the tree
plot(tree, cex=1)

# 5. Add tip labels colored by median fat content
plot(tree, cex=1) %>%
  tiplabels(
  labels = paste0(tree$tip.label, " — ", round(trait, 1), "%"),
  tip = 1:Ntip(tree),
  col = tip_colors,
  frame = "none",
  adj = -0.1,
...
```{r median_fat_by_animal}
library(dplyr)

median_fat_by_animal <- cheeses_clean %>%
  group_by(primarymilk) %>%
  summarize(median_fat = median(fat_pct, na.rm = TRUE))

median_fat_by_animal
```

```{r ggtree_plot, message=FALSE, warning=FALSE}
# If you don't have ggtree yet:
...
# check: label column should be cow/goat/sheep/...; median_fat is the median % for each

# 2. Add color gradient
pal <- colorRampPalette(c("gold", "orange", "red"))

# 3. Plot with ggtree
plot_fat <- ggtree(tree) %>% tree +
  geom_tippoint(aes(color = median_fat), size = 3) +
  geom_tiplab(offset = 0.05, size = 3) +
...
#### Plot heat map with animals and cheese family, and flavor